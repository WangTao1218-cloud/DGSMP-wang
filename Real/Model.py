...
class Decoding(nn.Module):
    ...
    def forward(self, E1, E2, E3, E4, E5):
        ...
        sz = E1.size()
        D4 = self.D4(torch.cat([E1, F.interpolate(D3, size=[sz[2],sz[3]], mode=self.upMode)], dim=1))

        ## estimating the regularization parameters w
        w = self.w_generator(D4)

        ## generate 3D filters
        f1 = self.filter_g_1(torch.cat([E1, F.interpolate(D3, size=[sz[2],sz[3]], mode=self.upMode)], dim=1))
        f2 = self.filter_g_2(torch.cat([E1, F.interpolate(D3, size=[sz[2],sz[3]], mode=self.upMode)], dim=1))
        f3 = self.filter_g_3(torch.cat([E1, F.interpolate(D3, size=[sz[2],sz[3]], mode=self.upMode)], dim=1))
        return w, f1, f2, f3
